                    ...
                    @if ($user->gallery)
                        <ul class="list-reset portfolio">
                            @foreach (json_decode($user->gallery, true) as $key => $galleryItem)
                                @php
                                    $cleanedUrl = explode('?', $galleryItem)[0]; // Убираем query параметры
                                    $isVideo = Str::contains($cleanedUrl, ['.mp4', '.mov', '.avi', '.mkv', 'video']);
                                    $isImage = Str::contains($cleanedUrl, [
                                        '.jpg',
                                        '.jpeg',
                                        '.png',
                                        '.gif',
                                ...
                                <li class="portfolio__item" data-graph-path="portfolio-item{{ $key }}">
                                    @if ($isImage)
                                        ...
                                        <x-cloudinary::image public-id="{{ $cleanedUrl }}"
                                            transformation="w_224,h_224,c_fill" />
                                    @elseif ($isVideo)
                                        <!-- Используем Cloudinary компонент для видео -->
                                        <x-cloudinary::video public-id="{{ $cleanedUrl }}"
                                            transformation="w_224,h_224,c_fill" controls />
                                    @endif
                                        ...
                                        <div class="swiper-slide">
                                            <div class="portfolio__slide">
                                                <div class="portfolio__item"
                                                    ...
                                                    @if ($isImage)
                                                        <!-- Используем Cloudinary компонент для изображения -->
                                                        <x-cloudinary::image public-id="{{ explode('?', $galleryItem)[1] }}"
                                                            transformation="w_224,h_224,c_fill" />
                                                    @elseif ($isVideo)
                                                        <!-- Используем Cloudinary компонент для видео -->
                                                        <x-cloudinary::video public-id="{{ $galleryItem }}"
                                                            transformation="w_224,h_224,c_fill" controls />
                                                    @endif
                                                    <button type="button" class="btn">
                                                        <span class="icon">

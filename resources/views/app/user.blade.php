                    ...
                    @if ($user->gallery)
                        <ul class="list-reset portfolio">
                            @foreach (json_decode($user->gallery, true) as $key => $galleryItem)
                                @php
                                    // Определяем, является ли файл видео или изображением
                                    $isVideo = Str::contains($galleryItem, ['.mp4', '.mov', '.avi', '.mkv', 'video']);
                                    $isImage = Str::contains($galleryItem, [
                                        '.jpg',
                                        '.jpeg',
                                        '.png',
                                        '.gif',
                                ...
                                @if ($isVideo)
                                    ...
                                    <video class="portfolio__item-video" width="224" height="224"
                                        src="{{ $galleryItem }}#t=0.001" preload="metadata" controls></video>
                                @elseif ($isImage)
                                    <picture class="portfolio__item-picture">
                                        <source srcset="{{ $galleryItem }}" type="image/webp">
                                        <img loading="lazy" src="{{ $galleryItem }}" class="portfolio__item-image"

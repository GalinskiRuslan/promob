...
@section('content')
    <main class="main"><h1 class="visually-hidden">Страница специалиста</h1>
        <div class="main-sections">
                        ...
                        <div class="card__header">
                            <div class="card__profile">
                                <picture class="card__profile-avatar">
                                    ...
                                    <img loading="lazy" src="{{ $user->photos ? asset($user->photos) : asset('./img/avatars/avatar-1.png') }}" class="image" width="142"
                                ...
                                <div class="card__profile-info">
                                    <h3 class="card__profile-name">
                                        ...
                                        @auth()
                                            @if($user->id == auth()->user()->id)
                                                <a style="display: flex; align-items: center;" href="{{ route('user_edit', auth()->user()->id) }}">
                                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M17.3076 2.99981C17.5703 2.73717 17.8821 2.52883 18.2252 2.38669C18.5684 2.24455 18.9362 2.17139 19.3076 2.17139C19.6791 2.17139 20.0468 2.24455 20.39 2.38669C20.7332 2.52883 21.045 2.73717 21.3076 2.99981C21.5703 3.26246 21.7786 3.57426 21.9207 3.91742C22.0629 4.26058 22.136 4.62838 22.136 4.99981C22.136 5.37125 22.0629 5.73905 21.9207 6.08221C21.7786 6.42537 21.5703 6.73717 21.3076 6.99981L7.80762 20.4998L2.30762 21.9998L3.80762 16.4998L17.3076 2.99981Z" stroke="#25282B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                                    </svg>
                                                </a>
                                            @endif
                                        @endauth
                                    ...
                                    <div class="card__profile-location"><span class="icon icon-color icon-lg"><svg><use
                                                    xlink:href="{{ asset('img/icons/location.svg#svg-location') }}"></use></svg> </span>{{$user->city->city}}
                                    </div>
                                    ...
                                    @auth()
                                        @if($user->id == auth()->user()->id)
                                            <div style="margin-top: 15px; flex-direction: column; align-items: flex-start;" class="card__info">
                                                ...
                                                <ul style="justify-content: flex-start;" class="list-reset card__language notranslate">
                                                    @if($user->language)
                        ...
                        <ul class="list-reset portfolio">
                                @foreach(json_decode($user->gallery, true) as $key => $galleryItem)
                                        ...
                                        @if(preg_match('/\.?(mp4|mov|avi)$/i', $galleryItem))
                                            <li class="portfolio__item" data-graph-path="portfolio-item{{ $key }}">
                                                <video class="portfolio__item-video" width="350" height="224" src="{{ asset($galleryItem) }}#t=0.001" preload="metadata"></video>
                                                <button type="button" class="btn">
                                                    <span class="icon">
                                                        <svg><use
                                                                xlink:href="img/icons/x.svg#svg-x"></use></svg>
                                            ...
                                            @if(preg_match('/_350x225\.(jpg|jpeg|png|gif|svg)$/i', $galleryItem))
                                                <div class="swiper-slide">
                                                    <div class="portfolio__slide">
                                                        <div class="portfolio__item" data-graph-path="portfolio-item{{ $key }}">
                                                                <picture class="portfolio__item-picture">
                                                                    <source srcset="{{ asset($galleryItem) }}" type="image/webp">
                                                                    <img loading="lazy" src="{{ asset($galleryItem) }}" class="portfolio__item-image" width="350" height="224" alt="Картинка">
                                                                </picture>
            ...
            <section class="section" style="margin-top: 0;">
                <div class="container">
                    ...
                    <div class="info">
                        <div class="info__price"><span class="icon icon-color icon-xxl"><svg><use
                                        xlink:href="{{ asset('img/icons/cost.svg#svg-cost') }}"></use></svg></span>
                            <div class="info__price-info"><h4 class="h2 info__price-title">Стоимость</h4>
                                <div class="info__price-value">от {{ $user->cost_from }} тг до {{$user->cost_up}}тг
                                </div>
                            </div>
                        </div>
                        <div class="info__description"><h3 class="h2 info__description-title">Детали работы</h3>
                            <p class="info__description-text">{{ $user->details ?? null}}</p></div>
                        <div class="info__description"><h3 class="h2 info__description-title">Обо мне</h3>
                            <p class="info__description-text">{{ $user->about_yourself ?? null }}</p></div>
                    </div>
                </div>
            </section>
            <section class="section">
                <div class="container">
                    @guest
                        <div class="section-header"><h2 class="h1 section-title">Отзывы
                                ({{$user->receivedComments->count() ?? 0}})</h2>
                            <button type="button" class="btn btn-primary btn-primary--theme btn-review"
                                    data-graph-path="modal-review-sign"><span class="icon"><svg><use
                                    ...
                                    <article class="card card--review card--light">
                                        <div class="card__body">
                                            <div class="card__body-start">
                                                ...
                                                <h3 class="h2 card__body-title" translate="no">{{ $comment->user->name }}</h3></div>
                                            <div class="card__body-end">
                                                <div class="card__body-header"><h3
                                                        class="h2 card__body-title" translate="no">{{ $comment->user->name }}</h3>
                                                ...
                                                <div class="card__body-main"><p
                                                        class="card__body-main_text">{{ $comment->result }}</p></div>
                                                <div class="card__time">Отзыв оставлен
                                                    <time
                                                        datetime="2023-12-14T12:00:00Z">{{ \Carbon\Carbon::parse($comment->created_at)->format('d.m.Y') }}</time>
                                                </div>
            ...
            {{--                    <div class="form-field form-field--send"><input type="text" class="field" name="result"--}}
            {{--                                                                    placeholder="Ввод">--}}
            {{--                        <button type="submit" class="btn form-field__btn"><span class="icon"><svg><use--}}
            {{--                                        xlink:href="{{ asset('img/icons/send.svg#svg-send') }}"></use></svg></span></button>--}}
            {{--                    </div>--}}
            {{--                </form>--}}
            {{--            </div>--}}
            {{--        </section>--}}
    ...
    @if($user->whatsapp || $user->instagram)
        <div class="graph-modal__container graph-modal__container--contacts" role="dialog" aria-modal="true"
            ...
            <button class="btn js-modal-close graph-modal__close" aria-label="Закрыть модальное окно"><span
                    class="icon"><svg><use xlink:href="{{ asset('img/icons/x.svg#svg-x') }}"></use></svg></span></button>
            <div class="graph-modal__content">
                <ul class="list-reset graph-modal__contacts">
                    @if($user->site)
                        <li>
                            <a href="https://{{$user->site}}" target="_blank" class="btn graph-modal__contacts-link"><span
                                    class="icon">
                    ...
                    @if($user->whatsapp)
                        <li>
                            <a href="https://wa.me/{{$user->whatsapp}}" target="_blank" class="btn graph-modal__contacts-link"><span
                                    class="icon"><svg><use
                                            xlink:href="{{ asset('img/icons/whatsapp.svg#svg-whatsapp') }}"></use></svg> </span>{{ $user->whatsapp }}
                            </a>
                        </li>
                    @endif
{{--                    <li><a href="{{$user->site}}" target="_blank" class="btn graph-modal__contacts-link">{{ $user->site }}</a></li>--}}
                    @if($user->instagram)
                        <li>
                            <a href="https://instagram.com/{{ $user->instagram }}" target="_blank" class="btn graph-modal__contacts-link"><span class="icon"><svg><use
                                            xlink:href="{{ asset('img/icons/instagram.svg#svg-instagram') }}"></use></svg> </span>{{ $user->instagram }}
                            </a>
                        </li>
                    @endif
                </ul>
